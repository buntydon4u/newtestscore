...
router.put('/addresses/:addressId', authMiddleware, asyncHandler(async (req, res) => {
    ...
    const address = oldAddress.find((a) => a.id === addressId);
    if (!address) {
...
router.post('/documents', authMiddleware, upload.single('file'), asyncHandler(async (req, res) => {
    if (!req.file) {
...
router.put('/documents/:documentId', authMiddleware, asyncHandler(async (req, res) => {
    const { documentId } = req.params;
    const oldDocument = await userService.getDocuments(req.user.userId);
    const document = oldDocument.find((d) => d.id === documentId);
    if (!document) {
...
router.delete('/documents/:documentId', authMiddleware, asyncHandler(async (req, res) => {
    const { documentId } = req.params;
    const documents = await userService.getDocuments(req.user.userId);
