...
import { cacheWrap } from '../redis/cacheWrap.js';
...
router.get('/', asyncHandler(async (req: any, res: any) => {
  ...
  // const data = await cacheWrap(`cache:list:courses:${filterHash}`, 300, async () => {
    ...
    const orderBy = sortBy ? { [sortBy as string]: sortOrder || 'asc' } : { createdAt: 'desc' };
    ...
    const data = { data: listData, total, page: parseInt(page as string), limit: parseInt(limit as string) };
  // });
