...
import { setOtp, getOtp, deleteOtp } from '../redis/otpService.js';
import { checkLoginRateLimit, checkOtpRateLimit } from '../redis/rateLimiter.js';
...
export class AuthService {
  ...
  async verifyResetOtp(email: string, otp: string): Promise<{ message: string; userId: string }> {
    const user = await prisma.user.findUnique({
      where: { email },
    });
